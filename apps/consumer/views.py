...
def dashboard(request):
    ...
    return render(
        ...
        "consumer/dashboard.html",
        {
            ...
            "searches": searches,
            "market_pulse": pulse,
            "nav_items": consumer_nav("dashboard"),
            "dashboard_eyebrow": "Consumer savings assistant",
            "dashboard_title": "Simple and fast price comparison for everyday buying",
            "dashboard_description": "Search Uzbek market products, compare marketplace prices, and save the best offers.",
        },
    )
...
def search(request):
    ...
    if form.is_valid():
        ...
        if q:
            queryset = queryset.filter(Q(title__icontains=q) | Q(model_name__icontains=q) | Q(brand__icontains=q))
        if brand:
        ...
        if request.user.is_authenticated and q:
            SearchHistory.objects.create(user=request.user, query=q, results_count=queryset.distinct().count())
    cards = []
    for product in queryset.distinct()[:12]:
        ...
        cards.append({"product": product, "analytics": analytics, "best_price": best_price, "summary": summary})
    return render(
        request,
        "consumer/search.html",
        {
            "form": form,
            "cards": cards,
            "nav_items": consumer_nav("search"),
            "dashboard_eyebrow": "Search intelligence",
            "dashboard_title": "Find fair prices in seconds",
            "dashboard_description": "Use filters for brand, category, location, and benchmark range.",
        },
    )
...
def product_detail(request, slug):
    ...
    return render(
        ...
        "consumer/product_detail.html",
        {
            ...
            "nav_items": consumer_nav("search"),
            "dashboard_eyebrow": "Product intelligence",
            "dashboard_title": product.title,
            "dashboard_description": "Historical price trend, marketplace comparison, and AI analyzer summary.",
        },
    )
...
def favorites(request):
    ...
    return render(
        request,
        "consumer/favorites.html",
        {
            "items": items,
            "nav_items": consumer_nav("favorites"),
            "dashboard_eyebrow": "Saved shortlist",
            "dashboard_title": "Favorites",
            "dashboard_description": "Track products you want to monitor or buy later.",
        },
    )
...
def profile(request):
    return render(
        ...
        {
            "nav_items": consumer_nav("profile"),
            "dashboard_eyebrow": "Account",
            "dashboard_title": "Profile",
            "dashboard_description": "Language, location, and consumer preferences.",
        },
    )
